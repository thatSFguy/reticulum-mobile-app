...
class AgnosticLoraRouterTest {
    ...
    fun announceWithNoPeersIsDeferredThenUnicastOnDiscovery() = runTest {
        ...
        val ev = r.onTextLine("  $peerId -> D97EEC3AD97EEC3AD97EEC3AD97EEC3A  ttl=595s", nowMs = 1_000)
        assertNotNull(ev)
        assertEquals(listOf("D97EEC3AD97EEC3AD97EEC3AD97EEC3A"), ev.newPeerNodes)
        assertNotNull(r.cachedAnnounceFor("D97EEC3AD97EEC3AD97EEC3AD97EEC3A"))
        // Only once per cached announce.
        assertNull(r.cachedAnnounceFor("D97EEC3AD97EEC3AD97EEC3AD97EEC3A"))
    }

    @Test
    fun freshAnnounceFansOutToAllKnownPeerNodesDeduped() = runTest {
        val r = router(fallback = "11223344112233441122334411223344")
        r.onTextLine("loc $peerId D97EEC3AD97EEC3AD97EEC3AD97EEC3A", nowMs = 0)
        r.onTextLine("loc ${"CC".repeat(16)} D97EEC3AD97EEC3AD97EEC3AD97EEC3A", nowMs = 0) // same node, second id
        r.onTextLine("loc ${"DD".repeat(16)} B51EEC13B51EEC13B51EEC13B51EEC13", nowMs = 0)
        val d = r.routeOutbound(announce(), nowMs = 1)
        assertIs<AgnosticLoraRouter.RouteDecision.Send>(d)
        assertEquals(setOf("D97EEC3AD97EEC3AD97EEC3AD97EEC3A", "B51EEC13B51EEC13B51EEC13B51EEC13", "11223344112233441122334411223344"), d.targets.toSet())
    ...
    fun dataToUnknownPeerBuffersThenFlushesOnLoc() = runTest {
        ...
        r.onTextLine("loc $peerId B51EEC13B51EEC13B51EEC13B51EEC13", nowMs = 1)
        val flushed = r.drainRoutable(nowMs = 2)
        assertEquals(1, flushed.size)
        assertContentEquals(raw, flushed[0].first)
        assertEquals("B51EEC13B51EEC13B51EEC13B51EEC13", flushed[0].second)
        assertTrue(!r.hasPending())
    }

    @Test
    fun unknownPeerFallsBackToConfiguredUplink() = runTest {
        val r = router(fallback = "9828f51b9828f51b9828f51b9828f51b") // lower-case input normalizes
        val d = r.routeOutbound(data(), nowMs = 0)
        assertIs<AgnosticLoraRouter.RouteDecision.Send>(d)
        assertEquals(listOf("9828F51B9828F51B9828F51B9828F51B"), d.targets)
    }
    ...
    fun inboundLinkRequestRoutesOurRepliesBack() = runTest {
        ...
        r.onInbound("B51EEC13B51EEC13B51EEC13B51EEC13", lr, nowMs = 0)
        val linkId = computeLinkId(parsePacket(lr)!!, crypto)
        ...
        assertIs<AgnosticLoraRouter.RouteDecision.Send>(d)
        assertEquals(listOf("B51EEC13B51EEC13B51EEC13B51EEC13"), d.targets)
    }

    @Test
    fun ownRegistrationEchoIsIgnored() = runTest {
        val r = router()
        assertNull(r.onTextLine("loc $selfId 9828F51B9828F51B9828F51B9828F51B", nowMs = 0))
        assertNull(r.onTextLine("  $selfId -> 9828F51B9828F51B9828F51B9828F51B  ttl=600s", nowMs = 0))
    ...
    fun registeredAckIsReportedButChangesNothing() = runTest {
        val r = router()
        val ev = r.onTextLine("registered 16-byte id at 9828F51B9828F51B9828F51B9828F51B", nowMs = 0)
        assertNotNull(ev)
    ...
    fun plainDestBroadcastsFanOutInsteadOfBuffering() = runTest {
        ...
        assertTrue(!r.hasPending())
        r.onTextLine("loc $peerId D97EEC3AD97EEC3AD97EEC3AD97EEC3A", nowMs = 1)
    ...
    fun registerAckLearnsAttachedNodeAndNeutralizesMatchingFallback() = runTest {
        ...
        val r = router(fallback = "9828F51B9828F51B9828F51B9828F51B")
        val ev = r.onTextLine("registered 16-byte id at 9828F51B9828F51B9828F51B9828F51B", nowMs = 0)
        assertNotNull(ev)
        assertTrue(ev.summary.contains("attached node 9828F51B9828F51B9828F51B9828F51B"))
        assertEquals("9828F51B9828F51B9828F51B9828F51B", r.attachedNodeHex)
        ...
        r.onTextLine("loc $peerId D97EEC3AD97EEC3AD97EEC3AD97EEC3A", nowMs = 2)
    ...
    fun heartbeatLearnsAttachedNodeSilently() = runTest {
        val r = router(fallback = "9828F51B9828F51B9828F51B9828F51B")
        assertNull(r.onTextLine("[hb] up=1616s  node=9828F51B9828F51B9828F51B9828F51B  nbrs=1 routes=2 txq=0 stk=1223", 0))
        assertEquals("9828F51B9828F51B9828F51B9828F51B", r.attachedNodeHex)
        assertIs<AgnosticLoraRouter.RouteDecision.Buffered>(r.routeOutbound(data(), nowMs = 1))
    }
    ...
    fun bindingAtAttachedNodeIsPurgedAndRejected() = runTest {
        ...
        val r = router()
        r.onTextLine("loc $peerId 9828F51B9828F51B9828F51B9828F51B", nowMs = 0) // learned before we know better
        r.onTextLine("registered 16-byte id at 9828F51B9828F51B9828F51B9828F51B", nowMs = 1) // purges it
        assertIs<AgnosticLoraRouter.RouteDecision.Buffered>(r.routeOutbound(data(), nowMs = 2))
        assertNull(r.onTextLine("loc $peerId 9828F51B9828F51B9828F51B9828F51B", nowMs = 3)) // rejected outright now
        assertTrue(r.knownPeerNodes().isEmpty())
    }
    ...
    fun loopbackInboundLearnsNothing() = runTest {
        ...
        val r = router()
        r.onTextLine("registered 16-byte id at D97EEC3AD97EEC3AD97EEC3AD97EEC3A", nowMs = 0)
        val lr = linkRequest(dest = selfHash)
        assertNull(r.onInbound("D97EEC3AD97EEC3AD97EEC3AD97EEC3A", lr, nowMs = 1))
        val linkId = computeLinkId(parsePacket(lr)!!, crypto)
    ...
    fun bufferedLinkPacketFlushesWhenTheLinkRequestPinArrives() = runTest {
        ...
        val ev = r.onInbound("B51EEC13B51EEC13B51EEC13B51EEC13", lr, nowMs = 1)
        assertNotNull(ev)
        assertTrue(ev.routesChanged)
        val flushed = r.drainRoutable(nowMs = 2)
        assertEquals(1, flushed.size)
    ...
    fun inboundDataPinsReverseRouteForDeliveryProof() = runTest {
        ...
        r.onInbound("D97EEC3AD97EEC3AD97EEC3AD97EEC3A", inboundData, nowMs = 0)
