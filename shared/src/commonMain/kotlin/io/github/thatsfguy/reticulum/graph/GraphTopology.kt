package io.github.thatsfguy.reticulum.graph

import io.github.thatsfguy.reticulum.store.StoredDestination
import io.github.thatsfguy.reticulum.transport.toHex

/**
 * Pure topology layer for the Graph tab — no Compose / SwiftUI / Color
 * dependencies, so it's testable in pure Kotlin and shared between the
 * Android Compose Canvas renderer and the iOS SwiftUI Canvas renderer.
 *
 * Promotes each unique `nextHop` (the first-hop transport_id captured
 * from inbound HEADER_2 announces, persisted on [StoredDestination] as
 * of v0.1.40) to its own synthetic relay node. Routes leaves through
 * their relay so the graph reflects "you reach Bob via R1" rather than
 * a flat star where every destination edged directly to "me".
 *
 * What we model:
 ...
 *   - Hops past the first relay. RNS only exposes the first transport_id
 *     to a leaf, so any 3+ hop path is shown as a single relay edge with
 *     a hop count label — the intermediate links are unknown and we
 *     don't pretend to know them. A future recursive heuristic could
 *     chain known relay announces, but that's deliberately out of scope
 *     here.
 *
 * Moved from `androidApp/.../ui/graph/GraphTopology.kt` to commonMain
 * 2026-05-08 so the iOS Graph tab can reuse the same topology
 * computation rather than maintaining a parallel Swift port. The
 * platform-specific renderers (Android Compose Canvas + iOS SwiftUI
 * Canvas) consume [Topology] directly.
 */

enum class NodeRole { ME, RELAY, LEAF }
...
fun buildTopology(
    ...
    // Index existing announces by dest_hash hex so we can resolve a
    // relay node's display name when its transport_id matches a
    // destination we've already seen (e.g. a transport node that also
    // announces itself).
    ...
    // to the matching announced destination (if present in byHash) or
    // to a synthetic "relay:<hex>" id for unannounced relays.
    ...
    // Second pass: emit leaf nodes + edges. A destination that was
    // already promoted to a relay in pass 1 doesn't get a second LEAF
    // node — its relay role wins to keep the graph from showing two
    // glyphs for the same dest_hash.
    ...
    // Third pass: edges. me → relay (one per relay), relay → leaf
    // (each multi-hop leaf to its relay), me → leaf direct (single-hop
    // leaves and the degraded fallback when nextHop is missing).
    if (ourHash != null) {
