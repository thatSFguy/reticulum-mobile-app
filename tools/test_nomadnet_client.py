...
Usage:
    python tools/test_nomadnet_client.py <node_dest_hash> [path[,path…]] [tcp_host:port]

Default path: /page/index.mu
Default TCP : 127.0.0.1:45420 — the loopback fixture node from
              tools/test_nomadnet_node.py. Pass a host:port to reach
              anything else; nothing here defaults onto the public mesh (#59).

Several comma-separated paths are fetched over ONE link, which is how you
check that every fixture page still serves after editing the node:

    python tools/test_nomadnet_client.py <hash> \
        /page/index.mu,/page/table.mu,/page/big.mu,/file/fixture.txt

With one path the body is printed; with several, a size summary is.

A path the node refuses (an ALLOW_LIST handler, when this client has not
identified) shows up as "no response" rather than an error: upstream answers
a denied REQUEST with silence, so the only signal is the receipt timing out.
...
def main():
    ...
    if len(node_hex) != 32:
        ...
        sys.exit(2)
    page_paths = [p for p in (sys.argv[2] if len(sys.argv) > 2 else "/page/index.mu").split(",") if p]
    tcp = sys.argv[3] if len(sys.argv) > 3 else "127.0.0.1:45420"
    ...
    # Rewritten every run, deliberately: the config is derived from the
    # argument, and a cached one is how "point it at localhost" silently keeps
    # dialling whatever host it was first told about.
    os.makedirs(CONFIG_DIR, exist_ok=True)
    config_path = os.path.join(CONFIG_DIR, "config")
    host, port = tcp.rsplit(":", 1) if ":" in tcp else (tcp, "4242")
    ...
    # The request in flight, so a link that drops mid-request fails THAT
    # request rather than hanging until its timeout.
    current = {"done": None, "holder": None}
    ...
    def link_closed(lnk):
        ...
        done, holder = current["done"], current["holder"]
        if done is not None and not done.is_set():
            holder["failed"] = True
            holder["reason"] = "link closed before response"
            done.set()
    ...
    for page_path in page_paths:
        ...
        def response_received(receipt, _done=request_done, _holder=response_holder):
            _holder["data"] = receipt.response
            _done.set()

        def response_failed(receipt, _done=request_done, _holder=response_holder):
            _holder["failed"] = True
            _holder["reason"] = f"request failed: {receipt.status}"
            _done.set()

        current["done"], current["holder"] = request_done, response_holder
        ...
        # A page over the link MDU comes back as a Resource, and a 256 KB
        # fixture takes longer than a single-packet RESPONSE.
        if not request_done.wait(timeout=120):
            results.append((page_path, None, "no response within 120s"))
            continue
        if response_holder["failed"]:
            results.append((page_path, None, response_holder["reason"]))
            continue
        results.append((page_path, response_holder["data"], None))
    ...
    print()
    print("=" * 64)
    for page_path, body, err in results:
        if err is not None:
            print(f"FAIL  {page_path}: {err}")
        else:
            size = len(body) if isinstance(body, (bytes, bytearray)) else len(str(body))
            first = ""
            if isinstance(body, (bytes, bytearray)):
                first = body.decode("utf-8", errors="replace").splitlines()[0][:48] if body else ""
            print(f"ok    {page_path}: {size} bytes  {first}")
    print("=" * 64)
