...
**Reticulum Relay Chat (experimental)** — opt-in IRC-style group chat layered on RNS Links — the [RRC protocol](https://rrc.kc1awv.net/), CBOR envelopes exchanged over an identified Link to a hub. Enable it in Settings to get a **Rooms** tab: add a hub by its destination hash, or one-tap-promote a discovered `rrc.hub` straight from the Nodes list (RRC hubs are recognized and labelled there), then connect, join rooms, and chat. The composer is a command line — type `/` for inline completion of the hub's commands, with `/join`, `/part`, `/nick`, `/me` and `/clear` handled client-side and everything else forwarded; the hub's replies render in the conversation rather than a banner. Rooms carry unread badges, a member roster, `@nick` / `@hashprefix` mention highlighting, and per-room notifications (all messages / mentions only / muted). Drafts and the room you are in survive switching tabs. Hub / room / message history persists in its own `rrc_*` tables. **Off by default and still under development** — not yet interop-verified against a live hub; ordinary LXMF messaging is unaffected whether it's on or off.
...
**Spec compliance & hardening** — §2.3 originator HEADER_1→HEADER_2 conversion (DATA + LINKREQ), §2.4 hop-count bound (packets with `hops ≥ 128` dropped as malformed; hop counts treated as approximate distance per the `local_hops_delta` privacy option), §5.7.2 LXMF proof-of-work stamps (generated on send against the recipient's advertised stamp cost, so modern Sideband doesn't down-trust our messages), §11.1 16-byte request_path_hash, §11.2 request_id verification, §6.5 link-addressed `dest_type = LINK`, §6.5.1 Ed25519-verified link DATA proofs, §6.7.1 initiator-side KEEPALIVE loop with Token-encrypted bodies, §6.7.3 LINKCLOSE body verification, §10.2 / §10.5 Resource framing + RESOURCE_REQ, §10.7 hashmap-continuation (HMU) discipline, §10.9 RESOURCE_ICL / RCL cancellation (both directions — cancelled transfers fail fast on both ends), validateAnnounce recomputes `SHA256(name_hash ‖ identity_hash)[:16]` to reject impersonation announces, Resource size + bz2 decompression-bomb caps. The spec baseline is synced to **RNS 1.4.1 / LXMF 1.1.0**, with no known surviving wire-format gaps.
...
A full audit landed 2026-05-13; this section summarizes its findings. **Zero CRITICAL issues, and every finding — HIGH, MEDIUM, and LOW — is now closed on both platforms.** Android shipped the HIGH items in v1.1.27 (auto-backup carve-out, lockscreen-notification privacy, passphrase-strength gate on `.rmid` export, Android Keystore-wrapping of the identity private keys); the last tracked follow-up, the iOS identity vault, shipped in ios-v1.0.90 (Keychain-held master key sealing the identity rows — see the [Identity key protection at rest](#identity-key-protection-at-rest) subsection for why a Keychain AES key is the idiomatic iOS equivalent of Android's Keystore key).
